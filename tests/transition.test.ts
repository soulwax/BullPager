...
const packet: Packet = {
  id: 'WARD-00', title: 'Record the transition charter', state: 'OPEN', owner: 'unassigned',
...
describe('transition preview', () => {
  ...
  it('changes only the selected packet block and never writes', () => {
    const source = '### WARD-00 — Record the transition charter\nID: WARD-00\nState: OPEN\nOwner: unassigned\nEvidence: none\nRemainder: none\n\n### WARD-01 — Another packet\nState: OPEN\n\n| WARD-00 | OPEN | unassigned | U0 | none |';
  ...
  it('synchronizes the selected ledger row in the proposal', () => {
    ...
    expect(proposed).toContain('| WARD-00 | ACTIVE | Ada | U0 | check-1 |');
  });
