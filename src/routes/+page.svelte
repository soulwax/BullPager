<script lang="ts">
  ...
  import { accessLabel, canCreateProjects, projectMark } from '$lib/projectAccess';
  ...
  const total = $derived(data.groups.reduce((sum, group) => sum + group.projects.length, 0));
  ...
  async function toggleStar(slug: string) {
    ...
    try {
      await fetch('?/toggleStar', { method: 'POST', body, headers: { accept: 'application/json' } });
    } finally {
      await invalidateAll();
    }
  }
...
<main class="board-home">
  <header class="topbar board-home-header">
    <div>
      ...
      <h1>{data.username ? `Welcome back, ${data.username}.` : 'Your boards'}</h1>
      <p class="subtitle">Every board you own, share, or can open — each with its own cards, files, and graph.</p>
    </div>
    <div class="top-links">
      {#if canCreate}<a class="primary-button" href="/projects/new"><Plus /> Create board</a>{/if}
    </div>
  </header>
  ...
  {#if total === 0}
    <section class="board-home-empty" aria-labelledby="first-board-title">
      <div>
        ...
        <p>Start from a template — it sets the columns, cadence, and look, and you can change all three later.</p>
      </div>
      {#if canCreate}
        <a class="primary-button" href="/projects/new"><Plus /> Choose a template</a>
      {:else}
        <p class="empty">An editor or administrator can create the first board.</p>
      {/if}
    </section>
  {:else}
      ...
      <section class="board-home-section" aria-labelledby={`group-${group.id}`}>
        ...
        <ul class="board-tile-grid">
          {#each group.projects as entry (entry.project.slug)}
            <li class="board-tile" class:starred={starred.has(entry.project.slug)}>
              <a class="board-tile-face" href={`/projects/${entry.project.slug}`}>
                <span class="board-tile-mark" aria-hidden="true">{projectMark(entry.project.name)}</span>
              ...
              <p class="board-tile-meta">
                <span class={`access-chip access-${entry.access}`}>{accessLabel[entry.access]}</span>
                <span>{entry.project.visibility === 'private' ? 'Private' : 'Shared'}</span>
                <span>owned by {entry.project.owner}</span>
              </p>
              <div class="board-tile-actions">
                <a class="quiet-button" href={`/projects/${entry.project.slug}`} aria-label={`Open the ${entry.project.name} board`}><LayoutGrid /> Board</a>
                <a class="quiet-button" href={`/projects/${entry.project.slug}/backlog`}><Rows3 /> Backlog</a>
                <a class="quiet-button" href={`/projects/${entry.project.slug}/files`}><Cloud /> Cloud</a>
                <a class="quiet-button" href={`/projects/${entry.project.slug}/graph`}><Waypoints /> Graph</a>
                {#if data.username}
                  <button
                    type="button"
                    ...
                    aria-label={starred.has(entry.project.slug) ? `Unstar ${entry.project.name}` : `Star ${entry.project.name}`}
                    onclick={() => toggleStar(entry.project.slug)}
                  ><Star fill={starred.has(entry.project.slug) ? 'currentColor' : 'none'} /></button>
                {/if}
              </div>
            </li>
          {/each}
    ...
    {#if canCreate}
      <a class="board-tile board-tile-new" href="/projects/new">
        ...
        <span class="board-tile-copy"><strong>Create a board</strong><small>Start from a template</small></span>
      </a>
    {/if}
  {/if}
</main>
