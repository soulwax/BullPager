...
<main class={`project-workspace theme-${setting('theme', 'midnight')} project-lane-${setting('lane_style', 'scroll')}`} class:has-project-background={Boolean(boardBackground.src)} class:has-color-background={Boolean(boardBackground.color)} style={boardSurfaceStyle}>
  ...
  {#if editingCard}
    ...
    <dialog open class="card-editor-modal" aria-labelledby="card-editor-title">
      <section class="card-editor-overview" aria-label="Card summary"><span class={`card-editor-state priority-${editingCard.priority}`}>{editingCard.priority}</span><span class="card-editor-lane">{editingCard.lane}</span><span>{editingCard.owner || 'Unassigned'}</span>{#if editingCard.startDate}<span class="start-date">Starts {dueLabel(editingCard.startDate)}</span>{/if}{#if editingCard.dueDate}<span class={`due-date due-${dueState(editingCard.dueDate)}`} class:due-complete={editingCard.dueComplete}>Due {dueLabel(editingCard.dueDate)}{editingCard.dueComplete ? ' · complete' : ''}</span>{/if}{#if editingCard.checklist.length}<span><Check /> {editingCard.checklist.filter((item) => item.done).length}/{editingCard.checklist.length}</span>{/if}<span>{commentsFor(editingCard.id).length} {commentsFor(editingCard.id).length === 1 ? 'comment' : 'comments'}</span></section>
