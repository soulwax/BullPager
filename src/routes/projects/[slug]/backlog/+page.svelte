<script lang="ts">
  ...
  const cards = $derived(
    data.cards.filter((card) => {
      const haystack =
        `${card.title} ${card.details} ${card.owner} ${card.tags.map((item) => item.name).join(" ")}`.toLowerCase();
      return (
...
<svelte:head
  ><title>{data.project.name} backlog · Cirrus Architecture Tool</title
  ></svelte:head
>
...
<main class="backlog-page">
  ...
  <section class="backlog-toolbar" aria-label="Backlog filters">
    <label
      >Search <input
        bind:value={query}
        placeholder="Title, detail, owner, or tag"
      /></label
    >
    <label
      >Lane <select bind:value={lane}
        ><option value="all">All lanes</option>{#each lanes as item}<option
            value={item}>{item}</option
          >{/each}</select
      ></label
    >
    <label
      >Tag <select bind:value={tag}
        ><option value="all">All tags</option>{#each data.tags as item}<option
    ...
    <span class="backlog-count"
      >{cards.length} of {data.cards.length} cards</span
    >
  </section>
  ...
  {#if cards.length === 0}
    <section class="project-filter-empty">
      ...
      <h2>No backlog cards match these filters.</h2>
      <p class="subtitle">Clear the search or broaden the selected lane/tag.</p>
    </section>
  {:else}
      ...
      {#each cards as card (card.id)}
        <li class:archived={card.archived} class="backlog-card">
          <div class="backlog-card-heading">
            <div>
              <p class="backlog-card-id">{card.id}</p>
              <h2>{card.title}</h2>
            </div>
            <a
              class="quiet-button"
              href={`/projects/${data.project.slug}?card=${encodeURIComponent(card.id)}`}
              >Open on board</a
            >
          </div>
          <div class="backlog-meta">
            <span>{card.lane}</span><span>{card.priority} priority</span><span
              >{card.owner || "unassigned"}</span
            >{#if card.dueDate}<span
                >Due {new Date(
                  `${card.dueDate}T00:00:00`,
                ).toLocaleDateString()}</span
              >{/if}{#if card.archived}<span>Archived</span>{/if}
          </div>
          {#if card.tags.length}<div class="backlog-tags">
              {#each card.tags as item}<span
                  style={`--tag-color: ${item.color}`}>{item.name}</span
                >{/each}
            </div>{/if}
          ...
          <details class="backlog-checklist" open={card.checklist.length > 0}>
            ...
            {#if card.checklist.length}<ol>
                {#each card.checklist as item}<li class:done={item.done}>
                    {#if item.done}<Check />{:else}<Circle />{/if}
                    {item.text}
                  </li>{/each}
              </ol>{:else}<p>
                No implementation checklist has been attached yet.
              </p>{/if}
          </details>
        </li>
      {/each}
