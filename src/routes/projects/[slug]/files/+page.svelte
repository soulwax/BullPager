...
<main class="project-files-page">
  ...
  <section class:drag-active={dragActive} class="files-workspace" aria-label="Project file workspace" ondragover={(event) => { event.preventDefault(); dragActive = true; }} ondragleave={() => { dragActive = false; }} ondrop={handleDrop}>
    {#if dragActive}<div class="files-drop-overlay" role="status"><span>↑</span><strong>Drop into {activeFolder === 'Root' ? 'project cloud' : activeFolder}</strong><small>Files stay scoped to this project</small></div>{/if}
    <aside class="files-sidebar">
      <div class="files-sidebar-heading"><div><p class="eyebrow">PROJECT DRIVE</p><h2>Files <small>{data.files.length}</small></h2><p class="drive-storage">{folderPaths.length - 1} folders · {fileSize(totalBytes)} stored</p></div>{#if data.canEdit}<button type="button" class="quiet-button" onclick={startNewFile}>+ New</button>{/if}</div>
      ...
      <nav class="drive-shortcuts" aria-label="Drive shortcuts"><button type="button" class:active={showRecent} onclick={selectRecent}><span>◷</span><span>Recent</span><small>{recentFiles.length}</small></button><button type="button" class:active={!showRecent && showAll && !search} onclick={selectAllFiles}><span>⌂</span><span>All files</span><small>{data.files.length}</small></button></nav>
      <nav class="folder-list" aria-label="Project folders">{#each folderPaths as folder}<button type="button" class:active={!showRecent && folder === activeFolder} onclick={() => selectFolder(folder)}><span>{folder === 'Root' ? '⌂' : '▸'}</span>{folder === 'Root' ? 'Root' : folder.split('/').at(-1)}<small>{folder !== 'Root' ? folder.split('/').slice(0, -1).join('/') : 'Project files'}</small></button>{/each}</nav>
    ...
    <section class="file-editor-panel">
      <div class="drive-toolbar"><div class="drive-breadcrumbs">{#if showRecent}<button type="button" onclick={selectRecent}>Recent</button>{:else if showAll}<button type="button" onclick={selectAllFiles}>All files</button>{:else}<button type="button" onclick={() => selectFolder('Root')}>Root</button>{#each folderTrail as folder}<span>/</span><button type="button" onclick={() => selectFolder(folder)}>{folder.split('/').at(-1)}</button>{/each}{/if}<span class="drive-count">{search ? `${visibleFiles.length} matches` : `${visibleFiles.length} files`}</span></div><div class="drive-controls"><label>Sort <select bind:value={sortMode} onchange={rememberDrivePreferences}><option value="modified-desc">Last modified</option><option value="modified-asc">First modified</option><option value="name-asc">A to Z</option><option value="name-desc">Z to A</option><option value="size-desc">Largest first</option><option value="size-asc">Smallest first</option></select></label><div class="view-switcher" role="group" aria-label="File view"><button type="button" class:active={viewMode === 'list'} aria-label="List view" onclick={() => { viewMode = 'list'; rememberDrivePreferences(); }}>☷</button><button type="button" class:active={viewMode === 'grid'} aria-label="Grid view" onclick={() => { viewMode = 'grid'; rememberDrivePreferences(); }}>▦</button><button type="button" class:active={viewMode === 'item'} aria-label="Item view" onclick={() => { viewMode = 'item'; rememberDrivePreferences(); }}>▤</button></div></div></div>
      {#if activeFile && !isTextFile}
        {@const visual = fileVisual(activeFile)}<div class="binary-file-preview"><p class="eyebrow">{isImageFile ? 'IMAGE PREVIEW' : 'FILE PREVIEW'}</p><h2>{activeFile.path}</h2>{#if isImageFile && activeFile.previewUrl}<img src={activeFile.previewUrl} alt={activeFile.path} />{:else}<div class={`binary-file-icon file-type-icon file-type-${visual.kind}`}><b>{visual.glyph}</b><small>{visual.label}</small></div>{/if}<p>{fileSize(activeFile.size)} · {activeFile.mimeType} · Modified {fileDate(activeFile.updatedAt)}</p><div class="file-action-strip"><a class="quiet-button" href={rawUrl(activeFile)} target="_blank" rel="noreferrer">Open / download</a><button type="button" class="quiet-button" onclick={() => copyFileLink(activeFile)}>Copy link</button><button type="button" class="quiet-button" onclick={() => copyFilePath(activeFile)}>Copy path</button></div></div>
