<script lang="ts">
  ...
  let {
    ...
    data: { templates: ProjectTemplate[] };
    form?: {
      error?: string;
      name?: string;
      slug?: string;
  ...
  let slug = $state("");
  $effect(() => {
    if (!data.templates.some((template) => template.id === selectedId))
      selectedId = data.templates[0].id;
...
<svelte:head
  ><title>New project · Cirrus Architecture Tool Board</title><meta
    name="description"
    content="Start a project from a focused workflow template."
  /></svelte:head
>
...
<main class="settings-shell new-project-shell">
  <header class="topbar">
    <div>
      ...
      <p class="subtitle">
        Choose a workflow that matches the work. Every project starts with one
        board, one project cloud, and an optional graph view; you can rename
        columns and adjust the look later.
      </p>
    </div>
  ...
  <form method="POST" class="new-project-form">
    <section class="settings-card project-identity-card">
      <div class="empty-slate-copy">
        ...
        <div>
          <p class="eyebrow">A CLEAR FIRST STEP</p>
          <h2>Name the workspace</h2>
          <p class="subtitle">
            Keep the name recognizable. The optional slug becomes the stable
            link you can share later.
          </p>
        </div>
      </div>
      <div class="project-fields">
        ...
        <label
          >URL slug <input
            ...
            placeholder="website-refresh (optional)"
          /><small>Leave blank to derive it from the name.</small></label
        >
      </div>
    </section>
    ...
    <section class="settings-card">
      <div class="settings-heading">
        <div>
          ...
          <p class="subtitle">
            Templates only set sensible defaults; they do not lock the project
            into one process.
          </p>
        </div>
        <span class="health valid">{data.templates.length} options</span>
      </div>
        ...
        {#each data.templates as template}
          ...
          >
            ...
            <span class="template-card-body"
              ><strong>{template.name}</strong><span>{template.summary}</span
              ><small>{template.bestFor}</small><span class="template-lanes"
                >{template.lanes.join(" → ")}</span
              ></span
            >
          </label>
        {/each}
    ...
    <section class="settings-card launch-card">
      <div>
        <p class="eyebrow">READY TO OPEN</p>
        <h2>{selected.name}</h2>
        <p class="subtitle">
          {selected.lanes.length} lanes · {selected.cadence} review rhythm · {selected.theme}
          visual baseline
        </p>
      </div>
