...
/**
 * The plan-file tool: the one place that writes packet state *back* into
 * `UNITY_PLAN.md`. Everything else in the app reads the plan or mirrors it
 * into the board database one way.
 *
 * It used to live at `/`, where its `load` unconditionally redirected to the
 * Unity board — so the page and its three transition actions were unreachable.
 * Moving it here makes the workflow usable again and frees `/` to be the
 * project list, which is what a board tool's front door should be.
 */
